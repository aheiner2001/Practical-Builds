...
# --- 4. LOGIN / JOIN LOGIC (WITH RESUME FIX) ---
            ...
            if st.form_submit_button("Start / Resume Fasting"):
                if u_name and u_group:
                    ...
                    if query.data and len(query.data) > 0:
                        ...
                        st.success(f"Welcome back, {u_name}! Resuming your fast...")
                    else:
                        # START NEW FAST
                        new_entry = {
                            "user_name": u_name, 
                        ...
                        st.success("Starting your new fast journey!")
                    
                    time.sleep(1)
                    st.rerun()
                else:
                    st.error("Please provide both a Name and Group Code.")
    with col_r:
        st.info("💡 **Tip:** If you've already started a fast, just enter your name and group code again to pick up where you left off.")
    st.stop()

# --- 5. DASHBOARD CALCULATIONS ---
user = st.session_state.user_data
# Ensure we handle timezone-aware datetimes for comparison
start_time = datetime.fromisoformat(user['start_time'].replace('Z', '+00:00'))
...
st.markdown(f"""
    <div style='grid-area: steps;'>
        ...
        <div style='background:#7cafc4; color:white; padding:15px; border-radius:10px;'>
            <strong>Electrolyte Guide:</strong><br>
            { "Full Pack Morning/Evening" if hours_passed > 36 else "1/2 Pack Now/Later" if hours_passed > 12 else "Water only for now" }
        </div>
    </div>
""", unsafe_allow_html=True)
...
st.subheader("👥 Live Group Progress")
...
for m in members.data:
    ...
    m_hours = (now - m_start).total_seconds() / 3600
