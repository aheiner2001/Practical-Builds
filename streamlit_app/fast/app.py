...
with col_main:
    ...
    cp_html = "".join([f"<div class='dot' style='left:{(hrs/user['target_hours'])*100}%; background:{'#0081ff' if my_hours>=hrs else '#e9ecef'};'></div><div class='dot-label' style='left:{(hrs/user['target_hours'])*100}%;'>{lab}</div>" for hrs, lab in checkpoints if (hrs/user['target_hours'])*100 <= 100])
