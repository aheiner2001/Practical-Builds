...
st.set_page_config(page_title="Car Count Predictor", layout="wide")

# ==========================================
# TITLE
# ==========================================
st.title("🚗 Car Count Prediction App")
st.write("Gradient Boosting model with weather + temporal features")
...
if st.button("📥 Load Data"):
    with st.spinner("Loading and cleaning data..."):
        ...
        df["hour_temp"] = df["hour_temp"].interpolate().bfill()
        df["hour_precip"] = df["hour_precip"].fillna(0)
        df["hour_wind"] = df["hour_wind"].interpolate().bfill()
        ...
        pd.Series(model.feature_importances_, index=X.columns).sort_values().tail(15).plot(kind='barh', ax=ax2)
        st.pyplot(fig2)
