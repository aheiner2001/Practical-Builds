...
.stApp { 
    background: linear-gradient(160deg, #e8f4fd 0%, #f0f8ff 40%, #e2eff9 100%); 
    min-height: 100vh; 
    color: #1a2a3a; 
}
...
div[data-testid="stButton"] > button {
    ...
    width: 100%; 
    box-shadow: 0 1px 4px rgba(30,111,168,0.08);
}
...
div[data-testid="stButton"] > button:hover { 
    ...
    box-shadow: 0 6px 20px rgba(30,111,168,0.25); 
}
...
.cal-cell { 
    background: white; 
    border: 1.5px solid #d4e9f7; 
    border-radius: 12px; 
    padding: 10px 6px 8px; 
    text-align: center; 
    min-height: 72px; 
    transition: all 0.18s ease; 
    box-shadow: 0 1px 3px rgba(30,111,168,0.06); 
    display: flex;
...
.cal-cell.has-slots:hover { 
    ...
    border-color: #1e6fa8; 
    transform: translateY(-2px); 
    box-shadow: 0 6px 18px rgba(30,111,168,0.18); 
}
...
.cal-cell.today { border-color: #f0a500 !important; border-width: 2px; }
.cal-cell.past { opacity: 0.38; pointer-events: none; }
...
/* --- 4. HERO & CARDS --- */
.hero-card { 
    background: linear-gradient(135deg, #1e6fa8 0%, #0d4f7c 100%); 
    border-radius: 20px; 
    padding: 2.2rem 2.5rem; 
    margin-bottom: 1.8rem; 
    position: relative; 
    overflow: hidden; 
    box-shadow: 0 8px 32px rgba(30,111,168,0.3); 
}
...
/* --- 5. MOBILE OVERRIDES (The "3-Day Row" Fix) --- */
@media (max-width: 672px) {
    /* Tighten container padding */
    .block-container { padding: 0.6rem !important; }

    /* Force Streamlit Columns to 3 per row */
    [data-testid="column"] {
    ...
    /* Shrink the Hero Card for screen real estate */
    .hero-card { padding: 1.5rem; margin-bottom: 1rem; }
    ...
    .day-num { font-size: 0.9rem !important; }
    .slot-count { font-size: 0.5rem !important; margin-top: 2px; }
    
    /* Remove dots on mobile for ultra-clean look */
