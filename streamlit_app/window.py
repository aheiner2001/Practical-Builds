import streamlit as st
from reportlab.lib.pagesizes import letter
from reportlab.lib import colors
from reportlab.lib.styles import getSampleStyleSheet, ParagraphStyle
from reportlab.platypus import SimpleDocTemplate, Paragraph, Spacer, Image, Table, TableStyle
from reportlab.lib.units import inch
from io import BytesIO
from PIL import Image as PILImage, ImageOps
import base64

# --- Minimalist Professional Palette ---
NAVY = colors.Color(0.05, 0.08, 0.12)   # Deep Navy for text
SLATE = colors.Color(0.44, 0.50, 0.56)  # Slate Grey for subtle borders
LIGHT_BG = colors.Color(0.98, 0.98, 0.98) # Almost white for background fills
WHITE = colors.white

def create_pdf(name, phone, notes, before_imgs, after_imgs):
    buffer = BytesIO()
    # Increased margins for a "breatheable" look
    doc = SimpleDocTemplate(buffer, pagesize=letter, rightMargin=50, leftMargin=50, topMargin=50, bottomMargin=70)
    story = []
    styles = getSampleStyleSheet()

    # --- Minimalist Styles ---
    title_style = ParagraphStyle(
        'TitleStyle', parent=styles['Heading1'], fontSize=24, textColor=NAVY, 
        spaceAfter=20, fontName='Helvetica-Bold', letterSpacing=1
    )
    
    # Clean, thin-bordered section header
    section_style = ParagraphStyle(
        'SectionStyle', parent=styles['Heading2'], fontSize=11, textColor=NAVY, 
        backColor=LIGHT_BG, borderColor=SLATE, borderWidth=0.5,
        borderPadding=10, spaceBefore=25, spaceAfter=15, fontName='Helvetica-Bold',
        borderRadius=2, textTransform='uppercase'
    )

    info_style = ParagraphStyle(
        'InfoStyle', parent=styles['Normal'], fontSize=10, leading=16, textColor=NAVY
    )

    # 1. Header Section
    story.append(Paragraph("SERVICE REPORT", title_style))
    
    review_url = "https://share.google/qJDmPYd7266QpvRys"
    
    # Service Details & Review Request
    service_text = f"""
    <b>Serviced by:</b> Aaron Heiner<br/>
    <b>Customer:</b> {name.upper()} &nbsp;&nbsp; | &nbsp;&nbsp; <b>Phone:</b> {phone}<br/><br/>
    It would help me a ton if you could leave a review for Glide Window Cleaning! If you could mention my name (Aaron) in the review, it helps me out even more. Thank you for your support!<br/>
    <a href="{review_url}" color="#2C3E50"><b><u>Please leave a review!</u></b></a>
    """
    
    story.append(Paragraph(service_text, info_style))
    story.append(Spacer(1, 0.1 * inch))

    # 2. JOB NOTES (Clean Box)
    if notes:
        story.append(Paragraph("JOB NOTES & OBSERVATIONS", section_style))
        notes_data = [[Paragraph(notes.replace('\n', '<br/>'), info_style)]]
        notes_table = Table(notes_data, colWidths=[6.8 * inch])
        notes_table.setStyle(TableStyle([
            ('BOX', (0,0), (-1,-1), 0.5, SLATE),
            ('LEFTPADDING', (0,0), (-1,-1), 15),
            ('TOPPADDING', (0,0), (-1,-1), 12),
            ('BOTTOMPADDING', (0,0), (-1,-1), 12),
            ('BACKGROUND', (0,0), (-1,-1), WHITE),
        ]))
        story.append(notes_table)

    # 3. Image Grid
    def add_image_section(label, files):
        if not files: return
        story.append(Paragraph(label, section_style))
        grid_data, row = [], []
        
        for uploaded_file in files:
            img = ImageOps.exif_transpose(PILImage.open(uploaded_file))
            img_tmp = BytesIO()
            img.save(img_tmp, format="PNG")
            img_tmp.seek(0)
            
            width = 3.2 * inch
            img_rl = Image(img_tmp, width=width, height=width * (img.height/img.width))
            row.append(img_rl)
            
            if len(row) == 2:
                grid_data.append(row)
                row = []
        
        if row: 
            row.append("")
            grid_data.append(row)

        t = Table(grid_data, colWidths=[3.4 * inch, 3.4 * inch])
        t.setStyle(TableStyle([
            ('ALIGN', (0,0), (-1,-1), 'CENTER'),
            ('VALIGN', (0,0), (-1,-1), 'MIDDLE'),
            ('BOTTOMPADDING', (0,0), (-1,-1), 20),
        ]))
        story.append(t)

    add_image_section("BEFORE PHOTOS", before_imgs)
    add_image_section("AFTER PHOTOS", after_imgs)

    # 4. Minimalist Footer
    story.append(Spacer(1, 0.5 * inch))
    footer_style = ParagraphStyle('F', parent=styles['Normal'], fontSize=10, textColor=SLATE, alignment=1)
    story.append(Paragraph("THANK YOU FOR YOUR BUSINESS • GLIDE WINDOW CLEANING", footer_style))

    doc.build(story)
    return buffer.getvalue()

# --- Streamlit UI ---
st.set_page_config(page_title="Service Report Generator", page_icon="📄")

# Clean UI Theme
st.markdown(f"""
    <style>
    .stButton>button {{ 
        background-color: #2C3E50;
        color: white; 
        width: 100%; 
        border-radius: 4px; 
        height: 3em; 
        font-weight: bold; 
        border: none;
    }}
    .stTextInput div[data-baseweb="input"], .stTextArea div[data-baseweb="base-input"] {{
        border-radius: 4px;
        border-color: #BDC3C7;
    }}
    .stTextInput div[data-baseweb="input"]:focus-within, 
    .stTextArea div[data-baseweb="base-input"]:focus-within {{
        border-color: #2C3E50 !important;
        box-shadow: none !important;
    }}
    header {{visibility: hidden;}}
    </style>
    """, unsafe_allow_html=True)

st.title("📄 Report Generator")

name = st.text_input("Customer Name")
phone = st.text_input("Customer Phone")
job_notes = st.text_area("Job Notes")

col1, col2 = st.columns(2)
with col1: before = st.file_uploader("Before Photos", accept_multiple_files=True)
with col2: after = st.file_uploader("After Photos", accept_multiple_files=True)

if st.button("GENERATE CLEAN PDF"):
    if name:
        with st.spinner("Generating clean report..."):
            pdf_data = create_pdf(name, phone, job_notes, before, after)
            st.success("Report Generated!")
            st.download_button("📥 Download PDF Report", pdf_data, f"Service_Report_{name}.pdf", "application/pdf")
            st.divider()
           
    else:
        st.error("Please enter a Customer Name.")
